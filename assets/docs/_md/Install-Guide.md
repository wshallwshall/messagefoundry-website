...
pip install "messagefoundry==0.2.1"          # pin the exact version (core runtime only)
```

> ⚠️ **Early access.** `0.2.1` is an **Early Access** release on public PyPI — feature-complete and
> test-validated, but the external review + pen test that gate a security-certified **v1.0** land after
> launch. The exact-pin command above (`==0.2.1`) resolves today; earlier releases such as `0.2.0` also
> remain installable (`pip install messagefoundry==0.2.0`). You can equally install from the engine's
...
```powershell
$V = "0.2.1"   # the exact version you intend to install
...
tag you are installing (e.g. `refs/tags/v0.2.1`).
...
├─ requirements.txt                # pins the engine:  messagefoundry==0.2.1
...
```powershell
pip install "messagefoundry[console]==0.2.1"     # PySide6 + keyring, into the same venv
...
```
        ...
        └───────────────┘  └───────────────┘  └───────────────┘
       engine 0.2.1 wheel  engine 0.2.1 wheel  engine 0.2.1 wheel  (pinned, identical)
```
...
1. Bump the pin in `requirements.txt` (e.g. `messagefoundry==0.2.1`) on a branch.
